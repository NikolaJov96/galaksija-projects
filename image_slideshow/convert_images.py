...
def image_to_galaksija(img_path: str | Path, dither: DitherMethod, multiplier: int = 1) -> tuple[list[int], PILImage]:
    ...
    """
    img_cols = SCREEN_WIDTH  * multiplier
    img_rows = SCREEN_HEIGHT * multiplier
    px_w     = PIXEL_WIDTH   * multiplier
    px_h     = PIXEL_HEIGHT  * multiplier
...
def write_header(output_dir: str | Path, num_images: int, multiplier: int = 1) -> None:
    with open(Path(output_dir) / "images.h", "w", encoding="utf-8") as f:
        ...
        f.write(f"#define MULTIPLIER     {multiplier}\n")
        f.write(f"#define IMAGE_COLS     (SCREEN_WIDTH  * MULTIPLIER)\n")
        f.write(f"#define IMAGE_ROWS     (SCREEN_HEIGHT * MULTIPLIER)\n")
...
def write_source(output_dir: str | Path, image_paths: list[Path], all_images: list[list[int]], multiplier: int = 1) -> None:
    img_cols = SCREEN_WIDTH  * multiplier
    img_rows = SCREEN_HEIGHT * multiplier
